...
def GetTotal(session,payload,headers):
	r = session.post('https://www.gelbeseiten.de/AjaxSuche',headers=headers,data=payload)
	json_data = json.loads(r.text)
	return	json_data['gesamtanzahlTreffer']


def Scrape(position, s,payload,headers):
	r = s.post('https://www.gelbeseiten.de/AjaxSuche',headers=headers,data=payload)
	json_data = json.loads(r.text)
	html = json_data['html']
...
def Scrape_Gel():
	...
	total = GetTotal(session,payload(1,city).formdata,headers)
	i = 1 
	alldf = []

	while i <= total:
		
		d = Scrape(i,session,payload(i,city).formdata,headers)
		df = pd.DataFrame(d)
		alldf.append(df)
